...
// opts.controls に要素（配列でも可）を渡すと、そこを押している間も上昇する。
// スマホで画面を親指で隠さずに操作するための操作バー用。
export function createGame(canvas, opts) {
  ...
  function reset() {
    ...
    S.enemies = []; S.shots = []; S.items = []; S.parts = [];
  ...
  /* 掛け声。敵に持たせておけば、敵が動いても吹き出しが付いてくる */
  function shout(enemy, text, dur) {
    enemy.speechText = text;
    enemy.speechT = dur || 1.4;
    enemy.speechMax = enemy.speechT;
  }
    ...
    for (const e of S.enemies) {
      ...
      if (e.type === "yoshino") {
        ...
        if (e.x < W - 130) {
          ...
          if (!e.said) { e.said = true; shout(e, "ヤマウラーー！！！", 1.8); }
        }
        ...
        if (!e.said && e.x < W - 120) { e.said = true; shout(e, "もてたい！", 1.5); }
      } else if (e.type === "fuse") {
  ...
  function drawWorld() {
    ...
    for (const e of S.enemies) {
      ...
      // 掛け声は毎フレーム敵の現在位置から描くので、ちゃんと付いてくる
      if (e.speechT > 0) {
        ctx.globalAlpha = Math.min(1, e.speechT / 0.35);
        speech(ctx, e.x, e.y - size * 0.52 - 26, e.speechText,
          Math.max(74, e.speechText.length * 14), 12);
        ctx.globalAlpha = 1;
      }
    }
  ...
  // 操作バーを押したときの動き。ゲーム中は上昇、メニュー中は主要ボタンと同じ扱い
  function primaryAction() {
    if (mode === "title" || mode === "gameover" || mode === "win") { reset(); mode = "playing"; }
  ...
  // 操作バーは複数渡せる（全画面の左右サム用）
  const pads = [].concat(opts.controls || []).filter(Boolean);
  ...
  for (const pad of pads) {
    ...
    pad.addEventListener("contextmenu", noMenu);
  }
  ...
  return {
    ...
    destroy() {
      ...
      for (const pad of pads) {
        ...
        pad.removeEventListener("contextmenu", noMenu);
      }
    },
  };
}
